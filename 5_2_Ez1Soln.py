...
beginfigure("5_2_Ez1Soln", 2*width, height)
...
def f(x):
    return -0.00833*x**6 + 0.15*x**5 -0.87813*x**4 + 1.38333*x**3 + 2.60625*x**2 - 3.25*x - 5.20833

graph = Graph(Function(f))
graph.setcolor(graphcolor)
graph.setlinewidth(graphwidth)
graph.setdomain([-1.25,6.75])
graph.draw()

point = Point([0.5, -6.06], 2)
point.setfillcolor(red)
point.fill()
point.draw()

point = Point([5, 6.33], 2)
point.setfillcolor(red)
point.fill()
point.draw()

label = Label(r"$(0.5, -6.06)$", [0.5, -6.06], alignment = "lt", offset = [2, -2] )
label.draw()

label = Label(r"$(5, 6.33)$", [5, 6.33], alignment = "ct", offset = [0, -2] )
label.draw()

label = Label(r"$y = F(x)$", [2.5,4.25], alignment = "lt", offset = [2, -2] )
label.draw()
