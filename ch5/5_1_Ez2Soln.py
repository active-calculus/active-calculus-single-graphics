...
beginfigure("5_1_Ez2Soln", 2*width, height)
...
axes.sethticks([0, 5, 30]) # you can do this in one line with setticks([], [])
axes.setvticks([0, 100, 300])
axes.drawticks()

axes.sethlabels([0, 10, 30])
axes.drawhlabels()
axes.setvlabels([0, 100, 300])
axes.drawvlabels()
...
label = Label("$C$", [20, 150], alignment = "lb", offset = [2,2])
label.draw()

label = Label("cal", [1, 325], alignment = "lb", offset = [2,2])
label.draw()
