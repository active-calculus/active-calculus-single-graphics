...
class Point(Area):
    def __init__(self, p, size = 2.5, color = 0, fillcolor = 0, linewidth=1):
        Area.__init__(self)
        self.color = color
        self.fillcolor = fillcolor
        self.linewidth = linewidth
...
class Axes(GeneralPath):
    ...
    def drawhlabels(self):
        ...
        while x <= right:
            if abs(x) < 0.000000001 and self.frame[0] == "c":
                x += dx
                continue
            s = str(x)
...
#            s = "$" + s + "$"
    ...
    def drawvlabels(self):
        ...
        if s < 0 or type(dy) is int:
            ...
            decimals = len(s) - s.find(".") - 1
        while y <= top:
            if abs(y) < 0.00000001 and self.frame[1] == "c":
                y += dy
                continue
            s = str(y)
            if s.find(".") < 0 and decimals > 0:
